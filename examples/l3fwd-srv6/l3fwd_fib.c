...
#define DEFAULT_OUTPUT_PORT 0

#define MAX_SID_ENTRIES 1000  // 定义 SID 表的最大条目数
...
struct sid_entry sid_table[MAX_SID_ENTRIES];
uint32_t sid_table_count = 0;
struct rte_hash *sid_hash_table;  // 用于加速 SID 查询的哈希表
...
void sid_table_init() {
    ...
    // 创建 SID 哈希表，加速 SID 查询
    struct rte_hash_parameters hash_params = {
        .name = "sid_hash_table",
        .entries = MAX_SID_ENTRIES,
        .key_len = sizeof(uint32_t),
        .hash_func = rte_jhash,
        .hash_func_init_val = 0,
    };
    sid_hash_table = rte_hash_create(&hash_params);
    if (sid_hash_table == NULL) {
        printf("Failed to create SID hash table.\n");
        // 可以添加错误处理的代码
    }
}

// 添加 SID 表条目
void sid_table_add_entry(uint32_t sid, uint8_t port) {
    if (sid_table_count < MAX_SID_ENTRIES) {
        sid_table[sid_table_count].sid = sid;
        sid_table[sid_table_count].port = port;
        sid_table_count++;

        // 将 SID 添加到哈希表中，以加速查询
        int ret = rte_hash_add_key(sid_hash_table, &sid);
        if (ret < 0) {
            printf("Failed to add SID to the hash table.\n");
            // 可以添加错误处理的代码
        }
    } else {
        printf("SID table is full, cannot add more entries.\n");
    }
}

// 根据 SID 查询出接口
uint8_t lookup_output_port(uint32_t sid) {
    int ret;
    uint32_t *sid_ptr = &sid;
    uint8_t port;

    ret = rte_hash_lookup(sid_hash_table, sid_ptr);
    if (ret < 0) {
        // 未找到匹配的 SID
        port = DEFAULT_OUTPUT_PORT;
    } else {
        uint32_t entry_index = ret;
        port = sid_table[entry_index].port;
    }
